...
const ComparisonChartComponent = memo(
  ({ yourScore, average, highest, total, width, t }) => {
    const barData = useMemo(() => {
      const safeTotal = total || 100;
      const yourPercent = parseFloat(
        ((yourScore / safeTotal) * 100).toFixed(2)
      );
      const avgPercent = parseFloat(((average / safeTotal) * 100).toFixed(2));
      const highestPercent = parseFloat(
        ((highest / safeTotal) * 100).toFixed(2)
      );
